...
import { createFound, deleteFound, getFoundById, updateFound } from "@/services/foundService";
...
const FoundFormScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isNew) {
        await createFound({
          title, description, location, address, category, phone, email,
          serverImageUrls, userId: user?.uid ?? "", status: "found",
        });
        Alert.alert("Success", "Found item added successfully!");
      } else {
        await updateFound(id!, {
          title, description, location, address, category, phone, email, serverImageUrls,
        });
  ...
  const handleDelete = () => {
    if (isNew || !id) return;
    Alert.alert("Delete Item", "Are you sure you want to delete this found item?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("Success", "Found item deleted successfully");
            router.replace("/(dashboard)/found");
          } catch (err) {
  ...
  return (
    <ScrollView className="flex-1 bg-darkBg p-5">
      {/* Page Title */}
      <Text className="text-3xl font-bold text-cyberGreen mb-6 text-center">
      ...
      {/* Location Dropdown */}
      <View className="mb-4">
        <Text className="text-sm font-semibold text-cyberMuted mb-2">District *</Text>
      ...
      {/* Where Found (Address) */}
      <View className="mb-4">
        <Text className="text-sm font-semibold text-cyberMuted mb-2">Where Found *</Text>
        <TextInput
          placeholder="Enter specific location where you found this item"
          placeholderTextColor="#8A99AD"
          multiline
          numberOfLines={3}
          className={`border p-4 rounded-xl bg-darkCard text-cyberText min-h-20 ${errors.address ? "border-cyberRose" : "border-darkBorder"}`}
          value={address}
          onChangeText={(text) => { setAddress(text); validateField("address", text); }}
          onBlur={() => validateField("address", address)}
        />
        {errors.address && <Text className="text-cyberRose text-xs mt-1">{errors.address}</Text>}
      </View>
      ...
      {/* Submit Button */}
      <TouchableOpacity
        className={`bg-cyberGreen rounded-xl p-4 mb-8 flex-row justify-center items-center shadow-[0_0_15px_rgba(0,230,118,0.3)] ${isSubmitting ? "opacity-70" : ""}`}
